...
- get a non-technical user from an empty project to the first successful
  translation without forcing them through advanced setup first

Flow:

1. User creates or opens a project.
2. Project readiness shows that configuration is incomplete.
3. User opens the project settings dialog from the blocker CTA or project chrome.
4. If app-level connections are missing, the project settings dialog deep-links to the app settings dialog.
5. User picks the services they already have, such as Gemini, OpenAI,
   DeepSeek, Anthropic, or a custom OpenAI-compatible endpoint.
6. User pastes API keys.
7. App tests capability coverage and shows a readiness summary.
8. App proposes a recommended workflow profile.
9. User accepts the recommended profile or edits it.
10. App saves reusable connections and returns the user to project settings.
11. User selects target language and project preset.
12. User chooses a shared workflow profile or creates a project-specific profile.
13. App returns the user to `Work`.
14. `Work` shows imported documents or prompts for import.
15. User imports documents.
16. `Work` shows the current blocking row or next row action.
17. User presses the row action, such as `Open OCR`, `Open Terms`, or `Open Translation`.
18. If the user needs document-specific detail, they open the document workspace.
19. When a document is ready, the user exports from the row or document workspace.
...
2. App lands on `Work`.
3. `Work` shows the context frontier, current blockers, and one primary action per row.
4. If setup changed globally and the project is now incomplete, the blocker target opens project settings.
...
## Journey 3: App Settings Wizard

Goal:

- let a user go from raw provider credentials to a usable default app settings state

Flow:

1. User opens the app settings dialog.
...
8. User either uses the recommendation, edits the workflow profile, or opens advanced settings.
9. App saves reusable connections and workflow profiles, then returns the user to the calling surface.

Success condition:

- the user can set up services without manually designing a workflow profile from scratch

## Journey 4: Project Settings

Goal:

- let a project adopt a shared workflow profile quickly while still supporting project-specific customization

Flow:

1. User opens the project settings dialog.
2. User sets target language.
3. User sets project preset.
4. Project settings shows the selected workflow profile and a short summary of covered steps.
5. User keeps the shared workflow profile or chooses to customize for this project.
6. If the needed connection or profile does not exist, user opens app settings from the dialog.
...
6. App offers explicit next actions such as `Rerun this page` or `Back to Work`.
...
3. If missing, CTA opens the relevant settings dialog.
...
3. If exporting from `Work`, the row-level `Export` action opens a small export dialog.
...
- project settings -> `Work` after successful project configuration
- project settings -> app settings when global connections are missing or need editing
- app settings -> return to the calling destination after save
...
the hub. `Terms`, document workspace, project settings, and app settings are
