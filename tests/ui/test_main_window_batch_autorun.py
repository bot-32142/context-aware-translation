...
def test_on_global_batch_worker_error_pauses_task_and_sets_backoff(tmp_path: Path):
    ...
    fake_window = SimpleNamespace(
        ...
        _global_batch_workers={"book-1": SimpleNamespace(task_id=task.task_id)},
    ...
    MainWindow._on_global_batch_worker_error(fake_window, "book-1", "boom")
    retry_after = fake_window._global_batch_retry_after.get("book-1")
    assert retry_after is not None
    assert retry_after > before
    fake_window._pause_global_task_after_worker_error.assert_called_once_with("book-1", "boom")
    ...
    MainWindow._pause_global_task_after_worker_error(pause_window, "book-1", "boom")
    # Close cached stores
