...
---指令---
1.  术语抽取与输出：
    *   抽取需要统一翻译的关键术语/短语（人物名、地名、组织、称号、技能/法术、物品/神器、专有设定、专有名词、在前后文中需要统一翻译的普通名词等等）。
    *   对每个术语提取：
        *   `term_name`：术语名称，保持原文形态。
        *   `term_description`： 用{source_language}，基于输入文本、用**第三人称**撰写的精炼且完整的描述。。如果该词为普通名词，描述中请明确其为术语的原因。
...
6.  排除项：
    *   不要输出：章节名、段落标题、前后文中可以翻译不一致的常用词/日常用语、、功能词/标点、纯数字/日期/货币/章节编号、无关叙述。
...
8.  提示：
    ...
    *   有些普通名词的不同目标语言中可能有多种表达方式，根据语境需要包含在术语表内以确保翻译一致性。
